...
%SQLCMD% -v quarter=%QTR% year=%YEAR% trade=1 -i %GITHUB%\ram\data\ern_pead\reconciliation\report_dates_prices.sql > %DATA%\earnings\implementation\reconcile\model_report_dates_prices.csv
%SQLCMD% -v quarter=%QTR% year=%YEAR% trade=2 -i %GITHUB%\ram\data\ern_pead\reconciliation\report_dates_prices.sql > %DATA%\pead\implementation\reconcile\model_report_dates_prices.csv
