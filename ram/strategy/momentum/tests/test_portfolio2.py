...
class TestPortfolio2(unittest.TestCase):
    ...
    def test_add_positions(self):
        ...
        port.add_positions(port_id=1, signals=signals, prices=prices)
        self.assertEqual(len(port.portfolios[1]), 4)
        port.close_portfolio(1)
        self.assertEqual(len(port.portfolios[1]), 0)
