...
class PortfolioConstructor1(Constructor):
    ...
    def get_args(self):
        return {
            'short_thresh': [.005, .01, .015],
            'long_thresh': [.025, .035, .045],
            'pos_size': [.02, .03, .04],
    ...
    def get_position_sizes(self, scores, portfolio, close_seccodes, long_thresh,
                           short_thresh, pos_size, close_out=False):
        """
        ...
        new_longs = set(new_univ[new_univ.score >= long_thresh].index)
        new_shorts = set(new_univ[new_univ.score <= -short_thresh].index)
        #new_longs = set()
        if close_out:
