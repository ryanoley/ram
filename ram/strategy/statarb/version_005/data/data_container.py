...
class DataContainer(BaseDataContainer):
    ...
    def set_args(self,
        ...
        self._data_rank_flag = data_rank_flag
        train_data, test_data = self._get_train_test()
        if data_rank_flag == 1:
            self._features = self._features_acc + self._features_rank
        elif data_rank_flag == 2:
            self._features = self._features_acc + self._features_nonrank
        else:
            self._features = self._features_acc + self._features_nonrank + self._features_rank
    ...
    def process_training_data(self, data, market_data, time_index):
        ...
        features_t1 = [x+'_nonrank' for x in features_t1]
        tdata1.columns = tdata1.columns[:3].tolist() + features_t1
        ...
        pdata = data[['SecCode', 'Date', 'TestFlag',
                      'TimeIndex']].merge(adata).merge(tdata1).merge(tdata2).merge(responses)

        self._features_acc = features_a
