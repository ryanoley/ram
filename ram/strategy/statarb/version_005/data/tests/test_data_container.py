...
from ram.strategy.statarb.version_005.data.data_container import *
...
class TestDataContainer(unittest.TestCase):
    ...
    def test_set_args(self):
        ...
        container._features_nonrank = ['A', 'B']
        container._features_rank = ['A_rank', 'B_rank']
        container._features_a = ['ACCT1', 'ACCT2']
        container.set_args(response_days=2,
                           response_type='Simple',
                           data_rank_flag=False)
        self.assertListEqual(range(8), container._train_data.index.tolist())
        self.assertListEqual(
            range(8), container._train_data_responses.index.tolist())
        self.assertListEqual(container._features,
                             ['ACCT1', 'ACCT2', 'A', 'B'])
    ...
    def test_process_live_data(self):
        ...
        benchmark = ['SecCode', 'Date', 'V1', 'V2', 'keep_inds', 'day_ret_nonrank',
                     'prma_2_nonrank', 'prma_5_nonrank', 'day_ret', 'prma_2',
                     'prma_5'] + ['binaries{}'.format(i) for i in range(16)]
        benchmark.sort()
